...
import { useVolume } from '../hooks/useAnalytics';
...
export default function VolumeChart() {
  ...
  const { data, isLoading: loading, error } = useVolume({
    protocol,
    interval,
    period,
  ...
  // Pivot rows by bucket so each bucket is one data point with per-token keys
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Transfer Volume" subtitle="Token volume over time by bucket interval" />
      ...
      <Card noPadding>
        <div className="p-5 border-b border-subtle">
          <h2 className="font-display text-sm font-semibold text-surface-100">
            Volume · {period.toUpperCase()} · {interval.toUpperCase()} intervals
          </h2>
        </div>
        <div className="p-5">
