...
export function patchWriteStreams({ getLines }) {
    ...
    const calculateLineCount = (lines) => {
        let count = 0;
    ...
    const clearAndWriteLines = (lines) => {
        if (isWriting && !lines)
            return;
    ...
    const handleOutput = function (buffer, encoding, cb) {
    ...
    process.stdout.write = handleOutput;
    process.stderr.write = handleOutput;
    ...
    if (isBun) {
        ...
        const makeConsoleWriter = (stream, _) => (...args) => {
            // biome-ignore lint/style/useTemplate:
            const formatted = util.format(...args) + "\n";
            // handleOutput so TUI is cleared + re-rendered
            handleOutput.call(stream, formatted, "utf8", undefined);
        };
        console.log = makeConsoleWriter(process.stdout, originalStdoutWrite);
        console.info = makeConsoleWriter(process.stdout, originalStdoutWrite);
        console.warn = makeConsoleWriter(process.stderr, originalStderrWrite);
        console.error = makeConsoleWriter(process.stderr, originalStderrWrite);
        shutdown = () => {
