...
import { getLiveQueryChannelName, getLiveQueryNotifyProcedureName, getLiveQueryNotifyTriggerName, getLiveQueryProcedureName, getLiveQueryTempTableName, getLiveQueryTriggerName, getPartitionName, getReorgProcedureName, getReorgTableName, getReorgTriggerName, getViewsLiveQueryNotifyTriggerName, } from '../drizzle/onchain.js';
...
export const createTriggers = async (qb, { tables, chainId }, context) => {
    await qb.transaction(async (tx) => {
        await Promise.all(tables.map(async (table) => {
            ...
            await tx.wrap({ label: "create_trigger" }, (tx) => tx.execute(`
...
export const dropTriggers = async (qb, { tables, chainId }, context) => {
    await qb.transaction(async (tx) => {
        await Promise.all(tables.map(async (table) => {
            const schema = getTableConfig(table).schema ?? "public";
            await tx.wrap({ label: "drop_trigger" }, (tx) => tx.execute(`DROP TRIGGER IF EXISTS "${getReorgTriggerName()}" ON "${schema}"."${chainId === undefined ? getTableName(table) : getPartitionName(table, chainId)}"`));
        }));
    }, undefined, context);
};
...
export const dropLiveQueryTriggers = async (qb, { namespaceBuild, tables, chainId, }, context) => {
    await qb.transaction(async (tx) => {
        const notifyTrigger = getLiveQueryNotifyTriggerName();
        await tx.wrap((tx) => tx.execute(`DROP TRIGGER IF EXISTS "${notifyTrigger}" ON "${namespaceBuild.schema}"."${PONDER_CHECKPOINT_TABLE_NAME}";`));
        const trigger = getLiveQueryTriggerName();
        for (const table of tables) {
            const schema = getTableConfig(table).schema ?? "public";
            await tx.wrap((tx) => tx.execute(`DROP TRIGGER IF EXISTS "${trigger}" ON "${schema}"."${chainId === undefined ? getTableName(table) : getPartitionName(table, chainId)}";`));
        }
    }, undefined, context);
};
...
export const createViews = async (qb, { tables, views, namespaceBuild, }, context) => {
    await qb.transaction({ label: "create_views" }, async (tx) => {
        await tx.wrap((tx) => tx.execute(`CREATE SCHEMA IF NOT EXISTS "${namespaceBuild.viewsSchema}"`));
        ...
        for (const table of tables) {
            await tx.wrap((tx) => tx.execute(`DROP VIEW IF EXISTS "${namespaceBuild.viewsSchema}"."${getTableName(table)}"`));
            await tx.wrap((tx) => tx.execute(`CREATE VIEW "${namespaceBuild.viewsSchema}"."${getTableName(table)}" AS SELECT * FROM "${namespaceBuild.schema}"."${getTableName(table)}"`));
        }
        for (const view of views) {
            await tx.wrap((tx) => tx.execute(`DROP VIEW IF EXISTS "${namespaceBuild.viewsSchema}"."${getViewName(view)}"`));
            await tx.wrap((tx) => tx.execute(`CREATE VIEW "${namespaceBuild.viewsSchema}"."${getViewName(view)}" AS SELECT * FROM "${namespaceBuild.schema}"."${getViewName(view)}"`));
        }
        await tx.wrap((tx) => tx.execute(`DROP VIEW IF EXISTS "${namespaceBuild.viewsSchema}"."${PONDER_META_TABLE_NAME}"`));
...
export const revertOmnichain = async (qb, { checkpoint, tables, }, context) => {
    ...
    return qb.transaction({ label: "revert" }, async (tx) => {
        ...
        for (const table of tables) {
            ...
            const schema = getTableConfig(table).schema ?? "public";
            const result = await tx.wrap((tx) => tx.execute(`
WITH reverted1 AS (
...
export const revertMultichain = async (qb, { checkpoint, tables, }, context) => {
    if (tables.length === 0)
        return [];
    return qb.transaction({ label: "revert" }, async (tx) => {
        const counts = [];
...
export const revertIsolated = async (qb, { checkpoint, tables, }, context) => {
    if (tables.length === 0)
        return [];
    return qb.transaction({ label: "revert" }, async (tx) => {
        const counts = [];
...
export const finalizeOmnichain = async (qb, { checkpoint, tables, namespaceBuild, }, context) => {
    const PONDER_CHECKPOINT = getPonderCheckpointTable(namespaceBuild.schema);
    // TODO(kyle) is this breaking an invariant?
    if (tables.length === 0) {
        await qb.wrap((db) => db
            .update(PONDER_CHECKPOINT)
            .set({ finalizedCheckpoint: checkpoint, safeCheckpoint: checkpoint }), context);
        return;
    }
    return qb.transaction({ label: "finalize" }, async (tx) => {
        await tx.wrap((tx) => tx.update(PONDER_CHECKPOINT).set({
...
export const finalizeMultichain = async (qb, { checkpoint, tables, namespaceBuild, }, context) => {
    const PONDER_CHECKPOINT = getPonderCheckpointTable(namespaceBuild.schema);
    // TODO(kyle) is this breaking an invariant?
    if (tables.length === 0) {
        await qb.wrap((db) => db
  ...
  WHERE chain_id = SUBSTRING(checkpoint, 11, 16)::numeric
    ...
    GROUP BY SUBSTRING(checkpoint, 11, 16)::numeric;`));
        for (const { chain_id, safe_checkpoint } of result.rows) {
            await tx.wrap((tx) => tx
                .update(PONDER_CHECKPOINT)
                .set({ safeCheckpoint: safe_checkpoint })
                .where(eq(PONDER_CHECKPOINT.chainId, chain_id)));
        }
    }, undefined, context);
};
export const finalizeIsolated = async (qb, { checkpoint, tables, namespaceBuild, }, context) => {
    const PONDER_CHECKPOINT = getPonderCheckpointTable(namespaceBuild.schema);
    ...
    if (tables.length === 0) {
        await qb.wrap((db) => db
            ...
            .where(eq(PONDER_CHECKPOINT.chainId, chainId)), context);
        return;
    }
    return qb.transaction({ label: "finalize" }, async (tx) => {
        await tx.wrap((tx) => tx
        ...
        for (const table of tables) {
            await tx.wrap((tx) => tx
                .delete(getReorgTable(table))
                .where(and(lte(getReorgTable(table).checkpoint, checkpoint), eq(sql `chain_id`, chainId))));
        }
    });
};
export const commitBlock = async (qb, { checkpoint, table, preBuild, }, context) => {
    ...
    if (preBuild.ordering === "experimental_isolated") {
        ...
        const chainId = Number(decodeCheckpoint(checkpoint).chainId);
        whereClause = and(eq(reorgTable.checkpoint, MAX_CHECKPOINT_STRING), eq(sql `chain_id`, chainId));
    }
    else {
...
WITH reverted1 AS (
  ...
  ON ${primaryKeyColumns.map(({ sql }) => `reverted2."${sql}" = reverted1."${sql}"`).join("AND ")}
  AND reverted2.operation_id = reverted1.operation_id
), ${getRevertSql({ table })}`), context);
};
const getRevertSql = ({ table }) => {
    const primaryKeyColumns = getPrimaryKeyColumns(table);
    const schema = getTableConfig(table).schema ?? "public";
    return `
inserted AS (
