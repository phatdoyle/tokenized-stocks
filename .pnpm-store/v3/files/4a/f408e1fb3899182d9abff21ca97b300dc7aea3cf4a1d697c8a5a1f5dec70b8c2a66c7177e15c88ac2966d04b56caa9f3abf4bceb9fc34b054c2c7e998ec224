...
import { BuildError, NonRetryableUserError, RetryableError, } from '../internal/errors.js';
...
export const createBuild = async ({ common, cliOptions, }) => {
    ...
    const executeFileWithTimeout = async ({ file, }) => {
        let timeoutId;
        const timeout = new Promise((resolve) => {
            timeoutId = setTimeout(() => resolve(new NonRetryableUserError("File execution did not complete (waited 10s)")), 10000);
        });
    ...
    const build = {
        ...
        async executeApi({ preBuild, configBuild, database }) {
        ...
        namespaceCompile() {
            ...
            if (viewsSchema === schema) {
                const error = new BuildError("Views schema cannot be the same as the schema.");
                error.stack = undefined;
                return { status: "error", error };
            }
            if (schema === "ponder_sync") {
                const error = new BuildError(`Invalid schema name. "ponder_sync" is a reserved schema name.`);
                error.stack = undefined;
                return { status: "error", error };
            }
            if (viewsSchema === "ponder_sync") {
                const error = new BuildError(`Invalid views schema name. "ponder_sync" is a reserved schema name.`);
                error.stack = undefined;
                return { status: "error", error };
            }
            if (schema.length > MAX_DATABASE_OBJECT_NAME_LENGTH) {
                const error = new BuildError(`Schema name cannot be longer than ${MAX_DATABASE_OBJECT_NAME_LENGTH} characters.`);
                error.stack = undefined;
                return { status: "error", error };
            }
            if (viewsSchema && viewsSchema.length > MAX_DATABASE_OBJECT_NAME_LENGTH) {
                const error = new BuildError(`Views schema name cannot be longer than ${MAX_DATABASE_OBJECT_NAME_LENGTH} characters.`);
                error.stack = undefined;
                return { status: "error", error };
            }
        ...
        async compileIndexing({ configResult, schemaResult, indexingResult, configBuild, }) {
            // Validates and builds the config
        ...
        async rpcDiagnostic({ configBuild }) {
            ...
            const results = await Promise.all(configBuild.rpcs.map(async (rpc, index) => {
                const chain = configBuild.chains[index];
                try {
                    const chainId = await rpc.request({ method: "eth_chainId" }, context);
        ...
        async databaseDiagnostic({ preBuild }) {
            ...
            if (dialect === "pglite") {
                ...
                try {
                    ...
                    const error = new RetryableError(`Failed to connect to PGlite database. Please check your database connection settings.\n\n${e.message}`);
                    error.stack = undefined;
                    return { status: "error", error };
                }
                finally {
                ...
                const pglitePath = pgliteDir === "memory://"
                    ? "memory://"
                ...
                try {
                    ...
                    const error = new RetryableError(`Failed to connect to database. Please check your database connection settings.\n\n${e.message}`);
                    error.stack = undefined;
                    return { status: "error", error };
                }
                finally {
