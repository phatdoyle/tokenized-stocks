...
import { MigrationError, NonRetryableUserError, ShutdownError, } from '../internal/errors.js';
...
import { eq, getTableName, getViewName, isTable, isView, sql, } from "drizzle-orm";
    ...
    if (dialect === "pglite" || dialect === "pglite_test") {
        ...
        common.shutdown.add(async () => {
            ...
            if (["start", "dev"].includes(common.options.command)) {
                await adminQB.wrap({ label: "unlock" }, (db) => db
                    .update(PONDER_META)
        ...
        syncQB = createQB(drizzleNodePg(driver.sync, {
            casing: "snake_case",
            schema: PONDER_SYNC,
        }), { common, isAdmin: false });
        adminQB = createQB(drizzleNodePg(driver.admin, {
            casing: "snake_case",
            schema: schemaBuild.schema,
        }), { common, isAdmin: true });
        userQB = createQB(drizzleNodePg(driver.user, {
            casing: "snake_case",
            schema: schemaBuild.schema,
        }), { common, isAdmin: false });
        readonlyQB = createQB(drizzleNodePg(driver.readonly, {
            casing: "snake_case",
            schema: schemaBuild.schema,
        }), { common, isAdmin: false });
        common.shutdown.add(async () => {
        ...
        async migrate({ buildId, chains, finalizedBlocks }) {
                ...
                for (let i = 0; i < schemaBuild.statements.tables.sql.length; i++) {
                    try {
                        ...
                        if (preBuild.ordering === "experimental_isolated" &&
                            ...
                            await tx.wrap((tx) => tx.execute(`${sql.slice(0, sql.length - 2)} PARTITION BY LIST (chain_id);`));
                            for (const chain of chains) {
                                await tx.wrap((tx) => tx.execute(`CREATE TABLE "${schemaName}"."${getPartitionName(tableName, chain.id)}" PARTITION OF "${schemaName}"."${tableName}" FOR VALUES IN (${chain.id});`));
                            }
                        }
                        else {
                            await tx.wrap((tx) => tx.execute(schemaBuild.statements.tables.sql[i]), context);
                        }
                    }
                    catch (_error) {
                        let error = _error;
                        if (!error.message.includes("already exists"))
                            throw error;
                        error = new MigrationError(`Unable to create table '${namespace.schema}'.'${schemaBuild.statements.tables.json[i].tableName}' because a table with that name already exists.`);
                        error.stack = undefined;
                        throw error;
                    }
                }
                for (const table of tables) {
                    await tx.wrap((tx) => tx.execute(`CREATE INDEX IF NOT EXISTS "${getTableName(table)}_checkpoint_index" ON "${namespace.schema}"."${getReorgTableName(table)}" ("checkpoint")`), context);
                }
            };
            const createViews = async (tx) => {
                for (let i = 0; i < schemaBuild.statements.views.sql.length; i++) {
                    await tx
                        .wrap((tx) => tx.execute(schemaBuild.statements.views.sql[i]), context)
                        .catch((_error) => {
                        const error = _error;
                        if (!error.message.includes("already exists"))
                            throw error;
                        const e = new MigrationError(`Unable to create view "${namespace.schema}"."${schemaBuild.statements.views.json[i].name}" because a view with that name already exists.`);
                        e.stack = undefined;
                        throw e;
                    });
                }
            ...
            const createEnums = async (tx) => {
                for (let i = 0; i < schemaBuild.statements.enums.sql.length; i++) {
                    await tx
                        .wrap((tx) => tx.execute(schemaBuild.statements.enums.sql[i]), context)
                        .catch((_error) => {
                        const error = _error;
                        if (!error.message.includes("already exists"))
                            throw error;
                        const e = new MigrationError(`Unable to create enum "${namespace.schema}"."${schemaBuild.statements.enums.json[i].name}" because an enum with that name already exists.`);
                        e.stack = undefined;
                        throw e;
                    });
                }
            };
            const createAdminObjects = async (tx) => {
...
CREATE TABLE IF NOT EXISTS "${namespace.schema}"."${PONDER_CHECKPOINT_TABLE_NAME}" (
  ...
  "finalized_checkpoint" VARCHAR(75) NOT NULL
                ...
                await tx.wrap((tx) => tx.execute(`CREATE SEQUENCE IF NOT EXISTS "${namespace.schema}"."${getReorgSequenceName()}" AS integer INCREMENT BY 1`), context);
            };
            const tryAcquireLockAndMigrate = () => adminQB.transaction({ label: "migrate" }, async (tx) => {
                await tx.wrap((tx) => tx.execute(`CREATE SCHEMA IF NOT EXISTS "${namespace.schema}"`), context);
                if (dialect === "pglite" || dialect === "pglite_test") {
                    await tx.wrap((tx) => tx.execute(`SET search_path TO "${namespace.schema}"`), context);
                }
                ...
                const previousApp = await tx.wrap((tx) => tx
                    .select({ value: PONDER_META.value })
                    .from(PONDER_META)
                    .where(eq(PONDER_META.key, "app"))
                    .then((result) => result[0]?.value), context);
                const metadata = {
                    version: VERSION,
                    build_id: buildId,
                ...
                if (previousApp === undefined) {
                    ...
                    await createLiveQueryProcedures(tx, { namespaceBuild: namespace }, context);
                    common.logger.info({
                    ...
                    await tx.wrap((tx) => tx.insert(PONDER_META).values({ key: "app", value: metadata }), context);
                    return {
                        status: "success",
                ...
                if (previousApp.is_dev === 1) {
                    ...
                    for (const table of previousApp.table_names ?? []) {
                        await tx.wrap((tx) => tx.execute(`DROP TABLE IF EXISTS "${namespace.schema}"."${table}" CASCADE`), context);
                        await tx.wrap((tx) => tx.execute(`DROP TABLE IF EXISTS "${namespace.schema}"."${getReorgTableName(table)}" CASCADE`), context);
                    }
                    for (const view of previousApp.view_names ?? []) {
                        await tx.wrap((tx) => tx.execute(`DROP VIEW IF EXISTS "${namespace.schema}"."${view}" CASCADE`), context);
                    }
                    for (const enumName of schemaBuild.statements.enums.json) {
                        await tx.wrap((tx) => tx.execute(`DROP TYPE IF EXISTS "${namespace.schema}"."${enumName.name}"`), context);
                    }
                    common.logger.warn({
                    ...
                    endClock = startClock();
                    await tx.wrap((tx) => tx.execute(`DROP TABLE IF EXISTS "${namespace.schema}"."${PONDER_CHECKPOINT_TABLE_NAME}" CASCADE`), context);
                    await tx.wrap((tx) => tx.execute(`DROP TABLE IF EXISTS "${namespace.schema}"."${PONDER_META_TABLE_NAME}" CASCADE`), context);
                    await tx.wrap((tx) => tx.execute(`DROP FUNCTION IF EXISTS "${namespace.schema}".${getLiveQueryProcedureName()}`));
                    await tx.wrap((tx) => tx.execute(`DROP FUNCTION IF EXISTS "${namespace.schema}".${getLiveQueryNotifyProcedureName()}`));
                    await createAdminObjects(tx);
                    await createLiveQueryProcedures(tx, { namespaceBuild: namespace }, context);
                    common.logger.debug({
                ...
                if (previousApp.version !== VERSION) {
                    const error = new MigrationError(`Schema "${namespace.schema}" was previously used by a Ponder app with a different minor version. Drop the schema first, or use a different schema. Read more: https://ponder.sh/docs/database#database-schema`);
                    error.stack = undefined;
                    throw error;
                }
                if (process.env.PONDER_EXPERIMENTAL_DB !== "platform" &&
                    ...
                    const error = new MigrationError(`Schema "${namespace.schema}" was previously used by a different Ponder app. Drop the schema first, or use a different schema. Read more: https://ponder.sh/docs/database#database-schema`);
                    error.stack = undefined;
                    throw error;
                }
                ...
                const checkpoints = await tx.wrap((tx) => tx.select().from(PONDER_CHECKPOINT), context);
                ...
                for (const { chainId, finalizedCheckpoint } of checkpoints) {
                    const finalizedBlock = finalizedBlocks[chains.findIndex((c) => c.id === chainId)];
                    if (hexToBigInt(finalizedBlock.timestamp) <
                        decodeCheckpoint(finalizedCheckpoint).blockTimestamp) {
                        throw new MigrationError(`Finalized block for chain "${chainId}" cannot move backwards`);
                    }
                }
                ...
                if (preBuild.ordering === "experimental_isolated") {
                    for (const { chainId } of checkpoints) {
                        ...
                        await dropLiveQueryTriggers(tx, { namespaceBuild: namespace, tables, chainId }, context);
                    }
                }
                else {
                    await dropTriggers(tx, { tables });
                    await dropLiveQueryTriggers(tx, { namespaceBuild: namespace, tables }, context);
                }
                // Remove indexes
                for (const indexStatement of schemaBuild.statements.indexes.json) {
                    await tx.wrap((tx) => tx.execute(`DROP INDEX IF EXISTS "${namespace.schema}"."${indexStatement.data.name}"`), context);
                }
                for (const table of tables) {
                ...
                await tx.wrap((tx) => tx.update(PONDER_META).set({ value: metadata }), context);
            ...
            if (result.status === "locked") {
                ...
                if (result.status === "locked") {
                    const error = new MigrationError(`Failed to acquire lock on schema "${namespace.schema}". A different Ponder app is actively using this schema.`);
                    error.stack = undefined;
                    throw error;
                }
            }
            heartbeatInterval = setInterval(async () => {
                try {
                    ...
                    const endClock = startClock();
                    await adminQB.wrap({ label: "update_heartbeat" }, (db) => db.update(PONDER_META).set({
