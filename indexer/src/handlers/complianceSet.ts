...
for (const contractName of CONTRACT_NAMES) {
  // Skip Robinhood contracts - they don't have ComplianceSet event in their ABI
  if (contractName.toLowerCase().endsWith("robinhood")) {
    continue;
  }
  
  ponder.on(`${contractName}:ComplianceSet`, async ({ event, context }) => {
  const { db } = context;
  ...
  await db.insert(schema.complianceSet).values({
    id: `${transaction.hash}-${log.logIndex}`,
    tokenName: metadata.name,
      contractAddress: log.address,
    tokenType: metadata.type,
    tokenTypeDetail: metadata.typeDetail,
    oldCompliance: args.oldCompliance,
    newCompliance: args.newCompliance,
